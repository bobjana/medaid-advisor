            ...
            val response =
                if (jsonModeEnabled) {
                    ...
                    chatClient
                        .prompt()
                        .system(
                            "You are a data extraction tool. Output ONLY valid JSON. No explanations, no markdown, no commentary. The context may contain Markdown-formatted tables with pipe separators (|). Parse table rows and columns carefully to extract structured data.",
                        ).user(prompt)
                        .options(jsonOptions)
                        .call()
                        .content()
